...
É bastante comum que precisemos repetir conjuntos de operações iguais para múltiplos vetores/objetos. Na medida em que o código cresce isso se torna um problema, pois para aplicar qualquer alteração que fizermos a um destes objetos a todos os outros, precisaremos reescrever o código para cada um deles. As funções nos ajudam em dois aspectos deste problema:
...
2.  Facilitam o debugging, uma vez que uma alteração feita na função mãe altera automaticamente os outputs resultantes de sua utilização.
